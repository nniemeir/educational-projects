...
// Serve the requested page and corresponding metadata
  ...
  if (strcmp(method, "GET") == 0) {
    ...
    while (bytes_read < file_size) {
      ...
      size_t chunk_size = file_size - bytes_read < available_space
                              ...
                              : available_space;
      size_t read =
          fread(response + metadata_length + bytes_read, 1, chunk_size, page);
      if (read == 0) {
        if (ferror(page)) {
...
char *generate_response(char *buffer) {
  ...
  if (!method) {
    free(response);
    return serve_page(method, "website/405.html", HTTP_405, response);
  }
  ...
  return serve_page(method, metadata,
