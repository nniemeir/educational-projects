...
void handling::take(MainWindow *mainWindow, QString target, Location* location) {
    ...
    if (itemIndex != -1) {
        ...
        player.setItemQuantity(playerItemIndex, player.getItemQuantity(playerItemIndex)+ 1);
        } else {
        ...
        if (location->getItemQuantity(itemIndex) == 1) {
            ...
            location->setItemQuantity(itemIndex, location->getItemQuantity(itemIndex) - 1);
        }
    } else {
