...
void handling::wear(MainWindow *mainWindow, QString target) {
    ...
    if (index != -1) {
        if (player.getItemEquipped(index) == 0) {
            ...
            mainWindow->setDescription(QString("You put on your %1.").arg(target.toLower()));
        } else {
            mainWindow->setDescription(QString("You are already wearing your %1.").arg(target.toLower()));
        }
    }
    else {
